...
- 陪審定理
    - 集団が$n$人からなり、それぞれが独立にある命題の真偽を判定する
    - 一人ひとりが0.5よりも大きい確率で正しい決定をできるならば、集団の人数$n$が増えると、多数決によって正しい判定を選択する確率は1に近づく
    - 一方で、一人ひとりが0.5よりも小さい確率で正しい決定をすると、多数決によって正しい判定を選択する確率は0に近づく
...
- 第２価格封印入札
    - 第２価格封印入札とは？
        - 互いに入札額がわからないようになっている
        - 一番高い入札額を提示した人が入札できる
        - ただしその入札額は第２位の人が提示した額とする
    - 例
        - Aの入札額：1,000円
        - Bの入札額：5,000円
        - Cの入札額：3,000円
        - 勝者は**B**で、支払う代金は２番目に高い入札額である**3,000円**となる
    - ゲーム理論のモデルで表現
        - 仮定
            - $n$人が参加するオークションに１つの財がかけられる。各プレイヤーは財に対する評価値$a_1, a_2, \cdots, a_n$を持つ。$a_i \geq 0$とし、プレイヤー$i$が払ってもよい上限額を表している。ただし、他のプレイヤーの評価値は観察できず、プレイヤー間で同じ評価値はないと仮定する。
            - 各プレイヤーは封印した入札額$x_i \geq 0$を１度だけ申告する。プレイヤーは他のプレイヤーの入札額を観察できない。最も大きな$x_i$を提示したプレイヤー$i$がオークションの勝者となる。
            - 全プレイヤーの入札額の組み合わせを$\boldsymbol{x} = (x_1, x_2, \cdots, x_n)$で表す。勝者$i$の支払額は、第２価格（２番目に大きい入札額）とし、$\boldsymbol{x}[2]$で表す。
            - 各プレイヤー$i$の利得は、入札額の組み合わせ$\boldsymbol{x}$のもとで、以下のようになる
                - $u_i(\boldsymbol{x}) = \left\{\begin{matrix} a_i - \boldsymbol{x}[2] & iが勝つとき \\ 0 & iが勝てないとき \end{matrix}\right.$
        - 命題
            - 各プレイヤーにとって、自分の評価値$a_i$を入札額として申告することが支配戦略となる。
        - 証明
            - 入札値$a_i$で$i$が勝つ場合
                - $a_i > \boldsymbol{x}[2]$が成り立っている
                - $i$の利得$a_i - \boldsymbol{x}[2]$は0より大きい
                - $a_i$以外の任意の入札額$x_i$を考え、$i$が戦略を変えても利得が増えないことを確認する
                1. 入札額$x_i$でもプレイヤー$i$が勝つ場合
                    - $a_i$以外の入札額$x_i$でも$i$が勝つ場合、支払額は$\boldsymbol{x}[2]$である
                    - 利得は$a_i - \boldsymbol{x}[2]$のまま
                    - 以下が成立する
                        - $u_i(a_i, \boldsymbol{s}_{-i}) \geq u_i(x_i, \boldsymbol{s}_{-i})$
                        - $a_i - \boldsymbol{x}[2] \geq a_i - \boldsymbol{x}[2]$
                2. 入札額$x_i$でプレイヤー$i$が負ける場合
                    - 勝てない場合の$i$の利得は0となる
                    - 以下が成立する
                        - $u_i(a_i, \boldsymbol{s}_{-i}) > u_i(x_i, \boldsymbol{s}_{-i})$
                        - $a_i - \boldsymbol{x}[2] > 0$
            - 入札値$a_i$で$i$が負ける場合
                - $a_i$で勝てない場合、プレイヤー$i$の利得は$u_i(a_i, \boldsymbol{s}_{-i}) = 0$
                - $a_i$以外の任意の入札額$x_i$を考え、$i$が戦略を変えても利得が増えないことを確認する
                1. $a_i$以外の$x_i$でも負ける場合
                    - 利得は0のまま変わらない
                2. $a_i$以外の$x_i$で勝つ場合
                    - $a_i$では勝てないので、$a_i \leq \boldsymbol{x}[2] \leq \boldsymbol{x}[1*]$が成立している
                    - $\boldsymbol{x}[1*]$は$a_i$で勝てない場合の最高入札額とする
                    - 勝つためには、$x_i > \boldsymbol{x}[1*]$を満たす$x_i$で入札せねばならない
                    - その場合に$i$の支払額は第２価格の$\boldsymbol{x}[1*]$となり、$a_i < \boldsymbol{x}[1*]$という関係から利得はマイナスとなる
                    - 以下が成立する
                        - $u_i(a_i, \boldsymbol{s}_{-i}) > u_i(x_i, \boldsymbol{s}_{-i})$
                        - $0 > a_i - \boldsymbol{x}[2]$
- まとめ
    - 第２価格封印入札は、「参加者に正直に評価額を入札させるためには、オークションのルールをどのように設定すればよいか？」という問題に対して理論的な解答を与えている
    - 適切なルール設定により、人々の選択をコントロールできる可能性がある
    - これを**メカニズムデザイン**という
